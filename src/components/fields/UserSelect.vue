    ...
    <div class="h-full">
      <div class="dropdown relative h-full">
        <button
          ...
          class="dropdown-toggle h-full px-6 overflow-hidden dark:bg-gray-800 w-full block rounded-md text-gray-500 dark:text-gray-400 transition duration-150 ease-in-out flex items-center whitespace-nowrap show"
          type="button"
        ...
        >
          ...
          <li v-for="user in filteredUsers" :key="user">
            <button
              ...
              class="dropdown-item text-sm py-2 px-4 font-normal block w-full bg-transparent text-gray-700 dark:text-gray-300 hover:bg-gray-100"
  ...
  methods: {
    async getUsers() {
      ...
      if (this.emptyOptions === true) {
        this.users.unshift({ id: "empty", name: "Без пользователя" });
      }
      this.$emit("wasLoaded", this.users);
      this.loading = false;
    },
    toggleDropdown() {
